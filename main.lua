...
function edit.modes.bone:do_gui()
    ...
    -- duplicate bone
    local function duplicate(bone, parent, src_polys, dst_polys)
        local k = Bone(bone.x, bone.y, bone.a)
        if parent then parent:add_kid(k) end
        for _, poly in ipairs(src_polys) do
            if poly.bone == bone then
                table.insert(dst_polys, {
                    data = table.clone(poly.data),
                    shade = poly.shade,
                    color = poly.color,
                    bone = k,
                })
        ...
        for i, l in ipairs(bone.kids) do
            duplicate(l, k, src_polys, dst_polys)
        end
        return k
    end
    ...
    if gui:button("copy") then
        if self.selected_bone ~= model.root then
            self.poly_buffer = {}
            self.bone_buffer = duplicate(self.selected_bone,
                                         nil,
                                         model.polys,
                                         self.poly_buffer)
        end
    ...
    if gui:button("paste") and self.bone_buffer then
        self.selected_bone = duplicate(self.bone_buffer,
                                       self.selected_bone,
                                       self.poly_buffer,
                                       model.polys)
